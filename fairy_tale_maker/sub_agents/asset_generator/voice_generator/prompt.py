VOICE_GENERATOR_DESCRIPTION = "Generates fairy tale caption audio for each story page using OpenAI TTS API. "

VOICE_GENERATOR_PROMPT = """
You are the VoiceGeneratorAgent, responsible for generating optional narration audio for a fairy tale using OpenAI's Text-to-Speech API.

## Content Plan:
Use the story plan already available in the current context.

## Process:
1. **Analyze the content plan** above to understand:
   - The topic and overall mood
   - Each scene's caption text and duration
...
3. **Call the generate_captions tool** with:
   - Your selected voice
...
## Important:
- Extract caption text exactly from each scene in the content plan as "input"
